...
- (void) loadView {
    ...
    CGRect labelFrame = CGRectMake(round(_blockerView.bounds.size.width/2 - width/2),
                                      round(_blockerView.bounds.size.height/2 - height/2),
                                      width,
                                      height);
    
	_blockerView = [[UIView alloc] initWithFrame: centeredFrame];
...
//	_blockerView.center = CGPointMake(self.view.bounds.size.width / 2, self.view.bounds.size.height / 2);
    ...
    label.center =CGPointMake(_blockerView.bounds.size.width / 2, _blockerView.bounds.size.height / 2 + height/4);

	[_blockerView addSubview: label];
	
	UIActivityIndicatorView				*spinner = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle: UIActivityIndicatorViewStyleWhite];
	
	spinner.center = CGPointMake(_blockerView.bounds.size.width / 2, _blockerView.bounds.size.height / 2 - height / 5);
