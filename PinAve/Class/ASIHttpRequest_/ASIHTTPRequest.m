...
- (void)startRequest
{
	...
	if (!validatesSecureCertificate) {
		CFReadStreamSetProperty((__bridge CFReadStreamRef)[self readStream], kCFStreamPropertySSLSettings, (__bridge CFTypeRef)([NSMutableDictionary dictionaryWithObject:(NSString *)kCFBooleanFalse forKey:(NSString *)kCFStreamSSLValidatesCertificateChain])); 
	}
	...
	if (oldStream) {
		CFStreamStatus status = CFReadStreamGetStatus((__bridge CFReadStreamRef)oldStream);
		if (status != kCFStreamStatusClosed && status != kCFStreamStatusError) {
			CFReadStreamClose((__bridge CFReadStreamRef)oldStream);
		}
		oldStream = nil;
	}
...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			[self setTotalBytesSent:[(__bridge_transfer NSNumber *)CFReadStreamCopyProperty((__bridge CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount) unsignedLongLongValue]];
...
- (void)readResponseHeaders
{
	...
	CFHTTPMessageRef headers = (CFHTTPMessageRef)CFReadStreamCopyProperty((__bridge CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPResponseHeader);
	if (!headers) {
		return;
	}
	if (CFHTTPMessageIsHeaderComplete(headers)) {
		...
		if (![self authenticationNeeded]) {
			...
			if (cLength) {
				SInt32 length = CFStringGetIntValue((__bridge CFStringRef)cLength);
...
- (void)handleBytesAvailable
{
    ...
    CFIndex bytesRead = CFReadStreamRead((__bridge CFReadStreamRef)[self readStream], buffer, sizeof(buffer));
    ...
    if ([self readStream]) {
		...
		if (![self canUsePersistentConnection]) {
			if ([self readStreamIsScheduled]) {
				CFReadStreamUnscheduleFromRunLoop((__bridge CFReadStreamRef)[self readStream], CFRunLoopGetCurrent(), kCFRunLoopDefaultMode);
			}
			CFStreamStatus status = CFReadStreamGetStatus((__bridge CFReadStreamRef)[self readStream]);
			if (status != kCFStreamStatusClosed && status != kCFStreamStatusError) {
...
- (void)scheduleReadStream
{
	if ([self readStream] && ![self readStreamIsScheduled]) {
		// Reset the timeout
		[self setLastActivityTime:[NSDate date]];
		CFReadStreamScheduleWithRunLoop((__bridge CFReadStreamRef)[self readStream], CFRunLoopGetCurrent(), kCFRunLoopDefaultMode);
...
+ (void)closePersistentConnection:(NSTimer *)timer
{
	...
	if (![connection objectForKey:@"request"] && [connection objectForKey:@"stream"]) {
		...
		#endif
		CFReadStreamRef stream = (__bridge CFReadStreamRef)[connection objectForKey:@"stream"];
